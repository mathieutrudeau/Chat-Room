...
print ("$test_dir/../Model/DB_user.php");
...
`mysql -u root < $test_dir/create_db_users.sql`;
...
# --------------------------------------------------------------------------
# DE/RE ACTIVATE USERS
# --------------------------------------------------------------------------
print "\n--------------------------------------------------------\n";
print "deactivate and reactivate users\n";
print "--------------------------------------------------------\n";

$dbh = connect($database);
ok (deactivate_user($dbh,'SaNdy'),"Deactivate 'SaNdy'");
ok (!user_exists($dbh,'sandy'),"'sandy no longer exists");
...
ok (reactivate_user($dbh,'Sandy'),"Reactivate user 'Sandy'");
ok(user_exists($dbh,'sandY'),"validate that 'sandY' now exists");
ok (deactivate_user($dbh,'sandy'),"Deactivate 'sandy' again");
ok(create_user($dbh,'sandy','newsandy','s@s'),'create New user "sandy"');
ok(validate_user($dbh,'sandy','newsandy'),'use new password to validate sandy');
ok(!reactivate_user($dbh,'sandy'),'try to reactivate sandy, returns false');
ok(validate_user($dbh,'sandy','newsandy'),'use new password to validate sandy again');
