...
function get_cookie_jar() {
       $root = get_web_root();
		if ( strlen($root) == 0) {
			$cookie_jar = 'C:\\xampp\\htdocs\\cookies.txt';
		}
		else {
			$cookie_jar = get_web_root()."/cookies.txt";
		}
		return $cookie_jar;
}
function remove_cookies ($cookie_jar = '') {
	$cookie_jar = get_cookie_jar();
    print("\n\nunlinking $cookie_jar\n\n");
	if (file_exists($cookie_jar)) {
		unlink ($cookie_jar);
	}
}
function post($data, $file, $cookie_jar = '') {
...
function get_web_root() {
    $web_root = getenv("SANDY_APACHE_ROOT");
	return $web_root;
}
...
function get_url_from_file ($file) {
    ...
    $current_machine=getenv("SANDY_APACHE_IP");
    $web_root=get_web_root();
	if (strlen($web_root) == 0) {
		$url = "http://localhost:444/".substr($file,16);
		$url = str_replace('\\', '/', $url);
	}
	else {
		$url = "http://".preg_replace(':\Q'.$web_root.':i',$current_machine, $file);
	}
